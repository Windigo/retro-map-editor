...
// ─── MapInit AB3 (constanten, tilesheet laden, data inlezen, BitMap bouwen) ────
/** Build mapinit.ab3 -- all AMIGA-mode setup: constants, Dim arrays, tilesheet IFF load,
 *  level data read, BitMap 1 build, sprite extraction. #CURRENT_LEVEL must be
 *  defined BEFORE the XINCLUDE in the calling file. */
function buildMapInitAb3(maps, bitplanes, spriteTileId, imageWidth, imageHeight) {
    if (maps.length === 0)
    ...
    const levelBlocks = Array.from({ length: totalLevels }, (_, lv) => {
        if (lv === 0) {
...
; gedefinieerd zijn vóór XINCLUDE "mapinit.ab3"
; ---------------------------------------------------------------
...
/** Build game.ab3 -- defines #CURRENT_LEVEL, includes mapinit.ab3, then BLITZ gameloop. */
function buildMultiGameAb3(totalLevels) {
    if (totalLevels === 0)
        return `; ERROR: no maps`;
    return `; XTRA
...
function initTiledViewerTab() {
    ...
    async function doTiledExport() {
        ...
        // Build mapinit.ab3 (AMIGA setup) with ALL maps
        const mapInitAb3Bytes = stringToAmigaBytes(buildMapInitAb3(allMaps, bp, 459, imgW, imgH));
        // Build game.ab3 (BLITZ gameloop, includes mapinit via XINCLUDE)
        const gameAb3Bytes = stringToAmigaBytes(buildMultiGameAb3(allMaps.length));
        const success = await editorApi.exportAmiga({
            ...
            mapInitAb3Data: Array.from(mapInitAb3Bytes),
