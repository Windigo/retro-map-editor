...
function buildMultiGameAb3(totalLevels: number): string {
  if (totalLevels === 0) return `; ERROR: no maps`;
...
function initTiledViewerTab(): void {
  ...
  async function doTiledExport(): Promise<void> {
    ...
    const mapInitAb3Bytes = stringToAmigaBytes(
      buildMapInitAb3(allMaps, bp, 459, imgW, imgH)
    );

    // Build game.ab3 (BLITZ gameloop, includes mapinit via XINCLUDE)
    const gameAb3Bytes = stringToAmigaBytes(
      buildMultiGameAb3(allMaps.length)
    );
